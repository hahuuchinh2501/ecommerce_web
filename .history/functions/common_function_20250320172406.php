...
function cart() {
    if(isset($_GET['add_to_cart'])) {
        ...
        if($num_of_rows > 0) {
            ...
            // Insert with user_id
            $insert_query = "INSERT INTO `cart_details` (product_id, ip_address, quantity, user_id) VALUES ($get_product_id, '$get_ip_add', 1, $user_id)";
...
function cart_item() {
    ...
    if(isset($_SESSION['user_id'])) {
        $user_id = $_SESSION['user_id'];
        $select_query = "SELECT * FROM `cart_details` WHERE ip_address='$get_ip_add' AND user_id=$user_id";
        $result_query = mysqli_query($con, $select_query);
        $count_cart_items = mysqli_num_rows($result_query);
    } else {
        // If not logged in, show 0 items
        $count_cart_items = 0;
    }
...
<?php
session_start(); // Đảm bảo session_start() được gọi ở đầu trang

function get_user_order_details(){
    global $con;

    // Kiểm tra và gỡ lỗi $_SESSION
    // error_log(print_r($_SESSION, TRUE)); // Ghi log $_SESSION để gỡ lỗi
    ...
    if(isset($_GET['user_id'])) {
        ...
        echo "<p class='text-center text-danger'>Please login to view orders</p>";
        return;
    }
    ...
    if(!isset($_GET['edit_account']) && !isset($_GET['my_orders']) && !isset($_GET['delete_account'])){
        ...
        if($row_count > 0){
            echo "<h3 class='text-center text-success mt-5 mb-2'>You have <span class='text-danger'>$row_count</span> pending orders</h3><p class='text-center'><a href='profile.php?my_orders&user_id=$user_id' class='text-dark'>Order Details</a></p>";
        } else {
